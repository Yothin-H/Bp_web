...
def predicted(test):
    ...
    data['Chromosome position'] = data['Chromosome position'].astype(int)
    ...
    # Check if another_param is a digit and less than or equal to the max of 'QUAL'
        if (not merged_df.empty) and (another_param.strip().isdigit() and (int(another_param) <= int(merged_df['QUAL'].max()))):
            aa2 = int(another_param)
        elif (not merged_df.empty) and (another_param.strip().isdigit() and (int(another_param) > int(merged_df['QUAL'].max()))):
            ...
            st.sidebar.info('No resistance found')
        else:
            pass
    except Exception as e:
        st.sidebar.error("An error occurred while processing the quality filter.")

    if (aa2 is not None) and (not merged_df.empty):
        merged_df2=merged_df.query('QUAL>=@aa2')
    elif (aa2 is None) and (not merged_df.empty) :
    ...
    if  not merged_df.empty :
        ...
        mm['POS'] = mm['POS'].astype(int)
        mm=mm.sort_values(by=['POS'])
